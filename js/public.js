$(document).ready(function() {
    ...
    $('body').on('click', '.archive-gallery-item-inner a', function(e) {
        ...
        $('.window-photo-slider-list').slick({
            ...
            slidesToShow: 1,
            slidesToScroll: 1,
            prevArrow: '<button type="button" class="slick-prev"><svg width="10" height="14" viewBox="0 0 10 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9 0.999265L2 6.99927L9 12.9993" stroke="white" stroke-width="2"/></svg></button>',
            nextArrow: '<button type="button" class="slick-next"><svg width="10" height="14" viewBox="0 0 10 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 13.0012L8 7.00122L0.999999 1.00122" stroke="white" stroke-width="2"/></svg></button>',
            dots: false,
            speed: 250,
    ...
    $('body').on('click', '.schedule-item-time a', function(e) {
        $('.video-detail-player').html('');
        $('.video-detail').removeClass('start');
        var curBlockVideo = $(this).parents().filter('.main-days-content').find('.video-detail');
        if (curBlockVideo.length == 1) {
            $('html, body').animate({'scrollTop': curBlockVideo.offset().top - 130});
            curBlockVideo.addClass('start');
            curBlockVideo.find('.video-detail-player').html('<iframe width="560" height="315" src="' + $(this).attr('href') + '&rel=0&autoplay=1" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>');
        }
        e.preventDefault();
    });
    ...
    $('body').on('click', '.schedule-item-record-play a', function(e) {
        var curRow = $(this).parents().filter('.schedule-item');
        var isFind = false;
        while (!isFind) {
            if (curRow.find('.schedule-item-time a').length == 1) {
                isFind = true;
                curRow.find('.schedule-item-time a').trigger('click');
            } else {
                curRow = curRow.prev();
                if (curRow.length == 0) {
                    isFind = true;
...
$(window).on('load resize scroll', function() {
    ...
    $('.main-days-video').each(function() {
        var curVideo = $(this);

        if ($(window).width() > 1169 && curVideo.parents().filter('.main-days-content').hasClass('active')) {
            if (windowScroll + $('header').height() + 20 > curVideo.offset().top) {
                curVideo.addClass('fixed');
                if (windowScroll + curVideo.find('.main-days-video-link').outerHeight() + $('header').outerHeight() + 20 > curVideo.parent().offset().top + curVideo.parent().outerHeight()) {
                    curVideo.find('.main-days-video-link').css({'margin-top': (curVideo.parent().offset().top + curVideo.parent().outerHeight()) - (windowScroll + curVideo.find('.main-days-video-link').outerHeight() + $('header').outerHeight() + 20)});
                } else {
                    curVideo.find('.main-days-video-link').css({'margin-top': 0})
                }
            } else {
                curVideo.removeClass('fixed');
                curVideo.find('.main-days-video-link').css({'margin-top': 0})
            }
        } else {
            curVideo.removeClass('fixed');
